...
Usage:
$ python3 get_data_color_splus.py /path/to/catalogue.csv /path/to/output_dir/
Your catalogue.csv must have #ID,RA,DEC,

  ____      _               _              _                                 
 / ___| ___| |_    ___ ___ | | ___  _ __  (_)_ __ ___   __ _  __ _  ___  ___ 
| |  _ / _ \ __|  / __/ _ \| |/ _ \| '__| | | '_ ` _ \ / _` |/ _` |/ _ \/ __|
| |_| |  __/ |_  | (_| (_) | | (_) | |    | | | | | | | (_| | (_| |  __/\__ \
 \____|\___|\__|  \___\___/|_|\___/|_|    |_|_| |_| |_|\__,_|\__, |\___||___/
                                                             |___/           
...
def grab_images():
    ...
    #     cat_all = pd.DataFrame()
        try:
            # print("Saving ID=",ID," RA,DEC=",RA,DEC)#
            png = conn.get_img(RA, DEC, int(Radius))#,stretch=1.0, Q=6) ## Get 12 band image RA, DEC, Radius
            # png = conn.twelve_band_img(RA, DEC, int(Radius),noise=0.03,saturation=0.03) ## Get 12 band image RA, DEC, Radius    
            png.save(save_path+"GRI/"+ID+"_gri.png")

    #             pf.writeto(save_path+ID2+"_R.fits",data,header=hdu[1].header,overwrite=True)
    #             print("Saving ID=",ID2," RA,DEC=",cat['RA'].iloc[0],cat['DEC'].iloc[0])
    #             cat_all = cat_all.append(cat)
        except:
    ...
    for band in BAND:
        ...
        def try_missing():
            ...
            for k in range(len(IDs)):
                ...
                if not os.path.exists(save_path+band+"/"+IDs[k]+"_gri.png"):
                    IDs_Re.append(IDs[k])
                    ra_Re.append(ra[k])
                    dec_Re.append(dec[k])
